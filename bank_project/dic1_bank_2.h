...
void phNumberValidation(char phNum[30]);
void copy_two_char_array(char receiver[200] ,char transmitter[200] );
void finding_phone_number(char tofind[30]);
...
void userSector(){
    ...
    if(user_option==1){
        ...
        while (phone_exist == -1) {
            ...
            scanf(" %[^\n]",input_ph_num);
            finding_phone_number(input_ph_num);
            if(phone_exist == -1) {
                printf("This phone number doesn't belong to any user.\n");
            }
        }
        printf("You are transferring money to %s.\n",db[phone_exist].name);
        ...
        transfer_money(emailExist,phone_exist,amount_to_transfer);
        printingAllData();
        userSector();



    } else if(user_option < 0 || user_option > 7){
        userSector();
    } else {
        printf("[-][-][-]: Invalid Key!Program Exited.\n");
        exit(99);
    }
...
void loadingAllDataFromFile(){
    ...
    if( fptr == NULL){
        ...
        for(int ncc=0; ncc<SIZE ; ncc++){
            ...
            fscanf(fptr ,"%d%s%s%s%s%s%s%u%u%f%s%d%s%u%s%d%s",&db[ncc].id,&db[ncc].name,&db[ncc].nrc,&db[ncc].email,
            ...
            for(int gcc=0; gcc<space_array[ncc]-16 ; gcc++){
...
void writingDataToFIle() {
    ...
    if(fptr == NULL) {
        printf("[-]Error writing data-s to the file.\n");
    } else {

        for(int ncc=0; ncc<users; ncc++) {
            fprintf(fptr,"%u%c%s%c%s%c%s%c%s%c%s%c%s%c%u%c%u%c%f%c%s%c%d%c%s%c%u%c%s%c%d",db[ncc].id,' ',db[ncc].name,' ',db[ncc].nrc,' ',db[ncc].email,' ',db[ncc].password,' ',
...
void printingAllData(){
    ...
    for(int ncc=0; ncc<users ; ncc++){
        ...
        printf("%u-%s-%s-%s-%s-%s-%s-%u-%u-%f-%s-%d-%s-%u-%s-%d",db[ncc].id,db[ncc].name,db[ncc].nrc,db[ncc].email,db[ncc].password,
               db[ncc].pOrb, db[ncc].loan_s,db[ncc].monthly_income,db[ncc].loan_amount ,db[ncc].loan_rate , db[ncc].acc_s ,db[ncc].acc_level,
               db[ncc].phNumber , db[ncc].cur_amount,db[ncc].address,db[ncc].transAmoLimitPerDay);
...
void phNumberValidation(char phNum[30]) {
    char ph_number_format[4] = {'+','9','5','9'};
    int format_char_count = 0;
    int ph_num_char_count = 0;
    int same_counting = 0;

    int input_ph_num = charCounting(phNum);

    for(int x=0; x<4; x++) {

        phNum[x] == ph_number_format[x] ? format_char_count++ : format_char_count;

    }
    if(format_char_count == 4) {
        for(int y=4; y<30; y++) {
            if(phNum[y] == '\0') {
                break;
            }
    ...
    for(int j=0; j<users; j++) {
        int users_ph_num = charCounting(db[j].phNumber);
        if(users_ph_num == input_ph_num) {
            for(int k=0; k<input_ph_num; k++) {
                if(phNum[k] == db[j].phNumber[k]) {
                    same_counting++;
                }
            }
        }
    ...
    if(same_counting != input_ph_num && ph_num_char_count == 9) {
        phone_valid = 1;
    } else {
        phone_valid = -1;
    }
...
void finding_phone_number(char tofind[30]) {
    ...
    int toCheckCount = charCounting(tofind);
    int sameCounting=0;

    for(int ncc=0; ncc<users ; ncc++){
        int dbPhoneCount = charCounting(db[ncc].email);

        sameCounting=0;
        if( toCheckCount == dbPhoneCount){

            for(int gcc=0; gcc<toCheckCount ; gcc++){

                if(tofind[gcc] != db[ncc].phNumber[gcc] ){
                    break;
                } else{
                    sameCounting++;
                }
